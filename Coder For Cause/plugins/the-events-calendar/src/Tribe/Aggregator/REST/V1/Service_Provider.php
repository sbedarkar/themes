...
class Tribe__Events__Aggregator__REST__V1__Service_Provider extends tad_DI52_ServiceProvider {
	...
	public function register_endpoints() {
		...
		register_rest_route( $this->namespace, '/import/(?P<import_id>\w+)/state', array(
			'methods' => WP_REST_Server::CREATABLE,
			'args' => $state_endpoint->CREATE_args(),
			'permission_callback' => array( $state_endpoint, 'can_create' ),
			'callback' => array( $state_endpoint, 'create' ),
		) );
	}
}
