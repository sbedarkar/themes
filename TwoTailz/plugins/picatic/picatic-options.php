...
add_action( 'admin_footer', 'picatic_settings_cache_clear' );

function picatic_settings_cache_clear()
{ ?>
  <script type="text/javascript" >
  function dismissCacheNotice(elm) {
    ...
    jQuery.post(ajaxurl, data, function(response) {
      cacheNotice = '\
        <div class="notice notice-success is-dismissible"> \
          ...
          <button type="button" OnClick="dismissCacheNotice(this)" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button> \
        </div>'
      jQuery('.options-page-title').after(cacheNotice);
    });
  }
  </script>
<?php }

add_action( 'wp_ajax_clear_cache', 'picatic_settings_cache_clear_callback' );

function picatic_settings_cache_clear_callback() {
  PicaticLib::cacheClear();
  wp_die();
}
...
function picatic_settings_cache_section_callback() {
  echo __( 'Caching will improve how quickly the Picatic widgets load.<br>If you are not seeing updated event content, clear the cache below to fetch new event information.', 'Picatic_Sell_Tickets_Widget_plugin' );
}
...
function picatic_cache_duration_render() {
  ...
  ?>
  <p>
    <input type="text" name="picatic_settings_cache[cache_duration]" value="<?php echo $options['cache_duration']; ?>" placeholder="<?php _e( 'seconds', 'Picatic_Sell_Tickets_Widget_plugin' ); ?>"/> (seconds)
  </p>
  <p>
    <button type="button" name="clear-cache" id="clear-cache" OnClick="clear_cache()" class="button button-primary">Clear cache</button>
  </p>
...
function picatic_options_page( $active_tab = '') {
  ?>
  <div class="wrap">
    <h2 class="options-page-title"><?php _e( 'Picatic Options', 'picatic' ); ?></h2>
    <?php
    if ( isset( $_GET[ 'tab' ] ) ) {
      $active_tab = $_GET['tab'];
