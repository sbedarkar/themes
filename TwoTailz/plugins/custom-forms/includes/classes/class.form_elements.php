	...
	class TC_Form_Elements {
		...
		function standard_field_label( $element_name, $front = false ) {
			if ( $front === false ) {
				...
				<input type="text" name="<?php echo $element_name; ?>[field_label][]" value="<?php echo esc_attr( isset( $this->id ) ? get_the_title( $this->id ) : ''  ); ?>" />
				<?php
			} else {
				return isset( $this->id ) ? get_the_title( $this->id ) : '';
			}
		}
		...
		function standard_field_required( $element_name, $front = false ) {
			...
			if ( $front === false ) {
				?>
				<label>
					<input type="checkbox" class="required_check" <?php checked( 1, $checked, true ); ?> />
		...
		function standard_field_export( $element_name, $front = false ) {
			...
			if ( $front === false ) {
				?>
				<label>
					<input type="checkbox" class="export_check" <?php checked( 1, $checked, true ); ?> />
		...
		function standard_field_as_ticket_template( $element_name, $front = false ) {
			...
			if ( $front === false ) {
				?>
				<label>
					<input type="checkbox" class="as_ticket_template_check" <?php checked( 1, $checked, true ); ?> />
		...
		function standard_field_choice_default_values( $element_name, $front = false ) {
			if ( $front === false ) {
				?>
				<label><?php _e( 'Default Value (which will be selected by default)', 'tc' ); ?></label>
		...
		function standard_field_admin_order_details( $element_name, $front = false ) {
			...
			if ( $front === false ) {
				?>
				<label>
					<input type="checkbox" class="order_details_check" <?php checked( 1, $checked, true ); ?> />
