...
class Petfinder_Listings_Featured_Pet extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        if( $instance["featured_pet_id"] != "" ){
            $response = wp_remote_get( "http://api.petfinder.com/pet.get?key=" . $petf_options["apikey"] . "&id=" . $instance["featured_pet_id"] );
            $xml = simplexml_load_string(wp_remote_retrieve_body($response));
			if( $xml->header->status->code != "100"){  //returns status code of 201 if not found
                $response = wp_remote_get("http://api.petfinder.com/pet.getRandom?key=" . $petf_options["apikey"] . "&shelterid=" . $petf_options["shelter_id"] . "&output=full");
				$xml = simplexml_load_string(wp_remote_retrieve_body($response));
			}
        }else{
            $response = wp_remote_get("http://api.petfinder.com/pet.getRandom?key=" . $petf_options["apikey"] . "&shelterid=" . $petf_options["shelter_id"] . "&output=full");
