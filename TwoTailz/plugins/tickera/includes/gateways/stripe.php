...
class TC_Gateway_Stripe extends TC_Gateway_API {
	...
	function on_creation() {
		...
		$this->method_img_url	 = $tc->plugin_url . 'images/gateways/stripe.png';
		$this->admin_img_url	 = $tc->plugin_url . 'images/gateways/small-stripe.png';
	...
	function payment_form( $cart ) {
        ...
        <tbody>
          ...
          <td>' . __( 'Cardholder Name:', 'tc' ) . '</td><td>
					<input id="cc_name" type="text" value="' . esc_attr( $name ) . '" /> </td>
          </tr>';
		$content .= '<tr>';
		$content .= '<td>';
	...
	function gateway_admin_settings( $settings, $visible ) {
		...
		<div id="<?php echo $this->plugin_name; ?>" class="postbox" <?php echo (!$visible ? 'style="display:none;"' : ''); ?>>
			...
			<div class="inside">
				<p class="description"><?php _e( "Accept Visa, MasterCard, American Express, Discover, JCB, and Diners Club cards directly on your site. Credit cards go directly to Stripe's secure environment, and never hit your servers so you can avoid most PCI requirements.", 'tc' ); ?> <a href="https://stripe.com/" target="_blank"><?php _e( 'More Info &raquo;', 'tc' ) ?></a></p>
				<table class="form-table">
					<tr valign="top">
						...
						<td>
							...
							<select name="tc[gateways][stripe][is_ssl]">
								<option value="1"<?php selected( $settings[ 'gateways' ][ 'stripe' ][ 'is_ssl' ], 1 ); ?>><?php _e( 'Force SSL (Live Site)', 'tc' ) ?></option>
								<option value="0"<?php selected( $settings[ 'gateways' ][ 'stripe' ][ 'is_ssl' ], 0 ); ?>><?php _e( 'No SSL (Testing)', 'tc' ) ?></option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<span class="description"><?php _e( 'You must login to Stripe to <a target="_blank" href="https://manage.stripe.com/#account/apikeys">get your API credentials</a>. You can enter your test credentials, then live ones when ready.', 'tc' ) ?></span>
							<p><label><?php _e( 'Secret key', 'tc' ) ?><br />
									<input value="<?php echo esc_attr( isset( $settings[ 'gateways' ][ 'stripe' ][ 'private_key' ] ) ? $settings[ 'gateways' ][ 'stripe' ][ 'private_key' ] : ''  ); ?>" size="70" name="tc[gateways][stripe][private_key]" type="text" />
								</label></p>
							<p><label><?php _e( 'Publishable key', 'tc' ) ?><br />
									<input value="<?php echo esc_attr( isset( $settings[ 'gateways' ][ 'stripe' ][ 'publishable_key' ] ) ? $settings[ 'gateways' ][ 'stripe' ][ 'publishable_key' ] : ''  ); ?>" size="70" name="tc[gateways][stripe][publishable_key]" type="text" />
								</label></p>
						</td>
					</tr>
					<tr valign="top">
						...
						<td>
							...
							<select name="tc[gateways][stripe][currency]">
								<?php
	...
	function process_payment( $cart ) {
		...
		try {
			$charge = Stripe_Charge::create( array(
				...
				"description"	 => apply_filters('stripe_order_description', sprintf( __( '%s Store Purchase - Order ID - %s', 'tc' ), get_bloginfo( 'name' ), $order_id, $buyer_email ), $order_id, $buyer_email),
